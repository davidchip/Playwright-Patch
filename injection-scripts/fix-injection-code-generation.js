...
// Create the final injection block that directly generates code
const injection = `
  
  // ANIMAKE INJECTION START
  console.log('🎭 Animake: Injecting code generation toolset...');
  ...
  try {
    await context.addInitScript(() => {
      ...
      console.log('🎭 Loading Animake Code Generation Tools...');
      ...
      function handleElementClick(e) {
        ...
        if (selectionMode === 'randomText') {
          generateRandomTextCode(element);
        } else if (selectionMode === 'randomInteger') {
          generateRandomIntegerCode(element);
        }
      }
      
      function generatePlaywrightSelector(element) {
        // Try to generate a Playwright-style selector
        
        // Check for accessible attributes first
        ...
        if (element.getAttribute('role')) {
          return 'page.getByRole("' + element.getAttribute('role') + '")';
        }
        ...
        // Check for parent text that might identify this field
      ...
      function generateRandomTextCode(element) {
        ...
        const code = 'const ' + variableName + ' = "' + value + '";\\nawait ' + selector + '.fill(' + variableName + ');';
        outputCode(code, 'Random Text');
        showToast('✅ Generated random text code!');
        
        // Also actually fill the element for visual feedback
      ...
      function generateRandomIntegerCode(element) {
        const value = Math.floor(Math.random() * 10000);
        const variableName = 'intVar_' + Math.random().toString(36).substring(2, 8);
        const selector = generatePlaywrightSelector(element);
        
        const code = 'const ' + variableName + ' = ' + value + ';\\nawait ' + selector + '.fill(String(' + variableName + '));';
        outputCode(code, 'Random Integer');
        showToast('✅ Generated random integer code!');
      ...
      function generateAssertVariable() {
        const variableName = prompt('Variable name to assert (e.g., textVar_abc123):');
        if (!variableName) return;
        
        const code = 'await expect(page.locator("*").filter({ hasText: ' + variableName + ' })).toBeVisible();';
      ...
      function init() {
        try {
          ...
          console.log('🎭 Animake Code Generation Tools loaded successfully!');
          showToast('🎉 Animake Code Tools Loaded!');
        } catch (error) {
    ...
    console.log('🎭 Animake: Code generation toolset injected successfully.');
  } catch (error) {
...
console.log('✅ Injected Animake code generation toolset with smart selectors.');
