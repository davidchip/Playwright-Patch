...
// Create the final injection block with properly escaped JavaScript
const injection = `
  ...
  try {
    await context.addInitScript(() => {
      ...
      function createToolButton(text, onClick) {
        ...
        button.style.cssText = 'background: linear-gradient(135deg, #0078d4, #106ebe); border: none; border-radius: 5px; color: white; padding: 8px 12px; cursor: pointer; font-size: 11px; text-align: left; width: 100%; transition: all 0.2s ease;';
      ...
      function showToast(message, type = 'success') {
        ...
        toast.style.cssText = 'position: fixed; bottom: 20px; right: 20px; background: ' + (type === 'success' ? '#107c10' : '#d13438') + '; color: white; padding: 12px 16px; border-radius: 6px; z-index: 1000000; transition: opacity 0.3s;';
      ...
      function outputCode(code, type) {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          ...
          showToast('Code logged to console', 'success');
        }
      }
      ...
      function init() {
        try {
          ...
          showToast('🎉 Animake Tools Loaded!');
        } catch (error) {
...
console.log('✅ Injected the full Animake toolset into Playwright (fixed escaping).');
