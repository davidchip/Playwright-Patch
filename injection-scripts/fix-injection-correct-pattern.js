// ANIMAKE INJECTION - CORRECT PATTERN VERSION
// This version uses improved regex patterns that capture the full selector chain
...
console.log('🎬 Loading Animake Pattern Replacer with Correct Patterns...');

// Monitor Playwright's code generation and replace patterns
let lastCodeContent = '';

function processGeneratedCode() {
  // Find Playwright's code area with comprehensive search
  const codeElements = [
    ...
    if (content && content.trim().length > 0) {
      ...
      if (content !== lastCodeContent) {
        ...
        if (content.includes('_random_text_') || content.includes('_random_int_')) {
          ...
          const randomTextPattern = /(await page\.[^;]+)\.fill\(['"]_random_text_(\d+)_['"]\);/g;
          ...
          const randomIntPattern = /(await page\.[^;]+)\.fill\(['"]_random_int_(\d+)_['"]\);/g;
...
// Also monitor on DOM changes with proper error handling
function setupMutationObserver() {
  if (document.body) {
    ...
    console.log('🔍 Animake: MutationObserver setup successfully');
  } else {
    console.log('⚠️ Animake: document.body not ready, retrying...');
...
console.log('🎬 Animake Pattern Replacer with Correct Patterns loaded successfully!');
